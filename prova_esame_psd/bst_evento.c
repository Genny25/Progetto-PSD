...
void stampa_evento_bst_nodi(eventBstNode node)
{
    if (node == NULL)
    {
        return;
    }
    stampa_evento_bst_nodi(node->left);
    stampa_evento(node->value);
    printf("\n\n");
    stampa_evento_bst_nodi(node->right);
}

void stampa_evento_bst(eventBst bst)
{
    stampa_evento_bst_nodi(bst->root);
}
