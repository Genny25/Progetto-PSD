...
int aggiungi_conferenza_evento(Conferenza conf)
{
    ...
    conf->id_evento++; // Incrementa l'ID degli eventi
...
int aggiungi_conferenza_sala(Conferenza conf)
{
    ...
    conf->id_sala++;                    // Incrementa l'ID delle sale
    return 0;
}
...
void salva_conferenza_su_file(Conferenza conf, FILE *file)
{
    ...
    {
        perror("Errore nella conferenza o nel file");
...
Conferenza leggi_conferenza_da_file(FILE *file)
{
    ...
    {
        perror("Errore durante la lettura del contatore ID evento e del contatore ID sala");
    ...
    if (conf == NULL)
    {
        perror("Errore nel creare una nuova conferenza");
